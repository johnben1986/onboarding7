...
const NewBlog = ({ editBlogSlug }) => {
  ...
  const createBlogPost = async () => {
    try{
      const response = await api.createOrUpdateBlogPost({
        ...blog,
        category_ids: tags,
      });
  ...
  return (
    <main className="bg-image3">
      {/* <Navbar title={`${blog.id ? "Edit" : "Create"} Blog Post`} /> */}
      <div className="mx-auto w-3/5 pb-24 font-brand-tomorrow text-lg text-white">
        <div className="my-12 border border-white rounded-lg p-3 space-y-3">
          ...
          <button
            className="gradient-color rounded-full text-brand-50 mt-0 inline-block px-5 py-3 font-brand-tomorrow text-lg"
